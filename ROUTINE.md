...
2. `candidates.json` lesen. **Höchstens 6** Kandidaten auswählen, die die Kriterien unten erfüllen. Gibt es weniger als 2 gute: keine Artikel schreiben, aber Schritt 5 bis 7 trotzdem ausführen.
...
7. Commit auf `main` mit Nachricht `Inhalte JJJJ-MM-TT: N neu` und pushen. `candidates.json` nicht committen.
